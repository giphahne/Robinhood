...
def ach(option):
    ...
    Combination of 3 ACH endpoints. Options include:
        ...
        * transfers
    '''
    return "https://api.robinhood.com/ach/iav/auth/" if option == "iav" else "https://api.robinhood.com/ach/{_option}/".format(
        _option=option)
...
def instruments(instrumentId=None, option=None):
    ...
    '''
    return "https://api.robinhood.com/instruments/{id}/".format(
        id=instrumentId) + "{_option}/".format(
            _option=option) if option else ""
...
def news(stock):
    return "https://api.robinhood.com/midlands/news/{_stock}/".format(
        _stock=stock)


def fundamentals(stock):
    return "https://api.robinhood.com/fundamentals/{_stock}/".format(
        _stock=stock)
...
def tags(tag=None):
    ...
    '''
    return "https://api.robinhood.com/midlands/tags/tag/{_tag}/".format(
        _tag=tag)


def chain(instrumentid):
    return "https://api.robinhood.com/options/chains/?equity_instrument_ids={_instrumentid}".format(
        _instrumentid=instrumentid)


def options(chainid, dates, option_type):
    return "https://api.robinhood.com/options/instruments/?chain_id={_chainid}&expiration_dates={_dates}&state=active&tradability=tradable&type={_type}".format(
        _chainid=chainid, _dates=dates, _type=option_type)


def market_data(optionid):
    return "https://api.robinhood.com/marketdata/options/{_optionid}/".format(
        _optionid=optionid)
